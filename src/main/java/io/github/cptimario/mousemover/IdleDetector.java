...
/**
 * Encapsulates idle heuristics used to decide whether to move the mouse.
 * Pure logic (no Robot calls) so it can be unit tested.
 */
public class IdleDetector {
    ...
    public IdleDetector(IdleTimeProvider idleProvider, int idleSeconds, int graceSeconds, int intervalSeconds, boolean fullscreenDetection, int edgeMargin, boolean micro, Random random) {
    ...
    public IdleDecision evaluate(Instant lastMovementAttempt, Point lastMousePosition, Dimension screenSize) {
        ...
        if (sinceLastMove <= intervalSeconds) {
            return new IdleDecision(false, "cooldown since last move=" + sinceLastMove + " <= interval=" + intervalSeconds);
        }
    ...
    /**
     * Notify the detector that activity has occurred (keyboard/mouse). This delegates to the provider.
     */
    ...
    private boolean isNearEdge(Point p, Dimension screen) {
        ...
        return p.x < margin || p.y < margin || p.x > screen.width - margin || p.y > screen.height - margin;
    }
    ...
    public static class IdleDecision {
        ...
        public IdleDecision(boolean shouldMove, String reason) {
